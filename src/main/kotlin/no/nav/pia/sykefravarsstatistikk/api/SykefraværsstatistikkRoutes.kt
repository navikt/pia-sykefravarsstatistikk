...
fun Route.sykefraværsstatistikk(
    ...
    get("/{orgnr}/sykefravarshistorikk/kvartalsvis") {
        val fnr = call.request.tokenSubject()
        // Ny måte å sjekke altinn på:
        val virksomheter = altinnTilgangerService.hentAlleOrgnr(call.request.hentToken())
        virksomheter?.forEach {
            println("[DEBUG][TEST] virksomhet bruker med fnr '$fnr' har tilgang til: '$it'")
        }
